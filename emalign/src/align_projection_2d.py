...
def align_projection(projs, vol, verbose=0, opt=None):
    ...
    Options:
        opt.sym- the symmetry type- 'Cn'\'Dn'\'T'\'O'\'I', where n is the
    ...
    if hasattr(opt, 'trueRots'):
        trueRots = opt.trueRots
    else:
        trueRots = None

    if hasattr(opt, 'trueRots_J'):
        trueRots_J = opt.trueRots_J
    else:
        trueRots_J = None

    if hasattr(opt, 'isshift'):
        isshift = opt.isshift
    else:
        isshift = 0

    if hasattr(opt, 'trueShifts'):
        trueShifts = opt.trueShifts
    else:
        trueShifts = None
    ...
    if trueRots is None:
        refrot = 0
    refrot_J = 1
    if trueRots_J is None:
        refrot_J = 0
    refshift = 1
    if trueShifts is None:
        refshift = 0
    canrots = 1
    if Rots is None:
        canrots = 0
    ...
    s_vec = np.linspace(-max_s, max_s, n_shifts).reshape(
        (1, n_shifts))  # those are the shifts in the r variable in the common lines.
    r_vec = np.arange(max_r).reshape((1, max_r))
    s_phases = np.exp(
        (-2 * math.pi * cmath.sqrt(-1)) / (2 * max_r + 1) * (r_vec.conj().T @ s_vec))  # size of (n_rXn_shift)
    ...
    else:  # added to fix a reference assignment warning - doesn't affect the algorithm
        err_Rots = np.array([])
    ...
    for projidx in range(n_projs):
        ...
        if isshift == 1:
            ...
            for j in idx:
                ...
                s_idx = np.array(np.where(temp_corr == temp_corr.max(axis=0, initial=0)))
