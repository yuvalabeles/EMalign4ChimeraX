...
def emalign(session, ref_map, query_map, downsample=64, projections=30, show_log=True, show_param=True, refine=False):
    ...
    threshold = query_map.minimum_surface_level
    ...
    if pixel_query > pixel_ref:
        ...
        # threshold = query_map.minimum_surface_level
        # # threshold_ref = ref_map.minimum_surface_level
        # # threshold = min(threshold_query, threshold_ref)
        #
        # log.info(f"threshold: {threshold}")
        #
        # query_vol_above_thresh = query_vol.copy()
        # ref_vol_above_thresh = ref_vol.copy()
        #
        # zero_count_query = np.count_nonzero(query_vol_above_thresh == 0)
        # log.info(f"zero_count query_map BEFORE: {zero_count_query}")
        # zero_count_ref = np.count_nonzero(ref_vol_above_thresh == 0)
        # log.info(f"zero_count ref_map BEFORE: {zero_count_ref}")
        #
        # query_vol_above_thresh[ref_vol < threshold] = 0
        # query_vol_above_thresh[query_vol < threshold] = 0
        # ref_vol_above_thresh[ref_vol < threshold] = 0
        # ref_vol_above_thresh[query_vol < threshold] = 0
        #
        # # add_vol = query_vol_above_thresh + ref_vol_above_thresh
        # # query_vol_above_thresh = query_vol.copy()
        # # ref_vol_above_thresh = ref_vol.copy()
        # # query_vol_above_thresh[add_vol == 0] = 0
        # # ref_vol_above_thresh[add_vol == 0] = 0
        #
        # zero_count_query = np.count_nonzero(query_vol_above_thresh == 0)
        # log.info(f"zero_count query_map AFTER: {zero_count_query}")
        # zero_count_ref = np.count_nonzero(ref_vol_above_thresh == 0)
        # log.info(f"zero_count ref_map AFTER: {zero_count_ref}")
        #
        # # Create GridData object with aligned query_vol but with the original query_map parameters:
        # ref_grid_data = arraygrid.ArrayGridData(ref_vol_above_thresh, origin=ref_dict.get("origin"),
        #                                         step=ref_dict.get("step"),
        #                                         cell_angles=ref_dict.get("cell_angles"),
        #                                         rotation=ref_dict.get("rotation"),
        #                                         symmetries=ref_dict.get("symmetries"),
        ...
        #
        # # Replace the data in the original query_map:
        # ref_map.replace_data(ref_grid_data)
        ################################################################################################################
        ...
        ################################################################################################################
        # query_vol_aligned = query_vol_above_thresh
        # bestR = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
        # bestdx = np.array([0, 0, 0])
        ################################################################################################################
        # from chimerax.map import high_indices
        # threshold = query_map.minimum_surface_level
        # log.info(f"threshold: {threshold}")
        # log.info(f"query_vol: {query_vol}")
        # log.info(f"shape of query_vol: {query_vol.shape}")
        # points_int = high_indices(query_vol, threshold)
        # log.info(f"points_int: {points_int}")
        # log.info(f"shape of points_int: {points_int.shape}")
        #
        # from numpy import single as floatc
        # points = points_int.astype(floatc)
        # weights = query_vol[points_int[:, 2], points_int[:, 1], points_int[:, 0]]
        # log.info(f"points: {points}")
        # log.info(f"shape of points: {points.shape}")
        # log.info(f"weights: {weights}")
        # log.info(f"shape of weights: {weights.shape}")
        #
        # vol_above_thresh = query_vol.copy()
        # zero_count = np.count_nonzero(vol_above_thresh == 0)
        # log.info(f"zero_count BEFORE: {zero_count}")
        # vol_above_thresh[vol_above_thresh < threshold] = 0
        # zero_count = np.count_nonzero(vol_above_thresh == 0)
        # log.info(f"zero_count AFTER: {zero_count}")
        # log.info(f"vol_above_thresh: {vol_above_thresh}")
        # log.info(f"shape of vol_above_thresh: {vol_above_thresh.shape}")
