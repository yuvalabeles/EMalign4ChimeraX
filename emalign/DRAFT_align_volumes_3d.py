...
def align_volumes(vol1, vol2, verbose=0, opt=None, show_log=True, show_param=True, session=None):
    ...
    n_1 = np.shape(vol1)
    n_2 = np.shape(vol2)
    log.info(f'Referencing vol1 ({n_1}) and vol2 ({n_2})')
    ...
    if n_1[0] == n_2[0]:
        ...
        if downsample is None:
            ...
            vol1_ds = cryo_downsample(vol1, (n_ds, n_ds, n_ds))
            vol2_ds = cryo_downsample(vol2, (n_ds, n_ds, n_ds))
        log.info(f'Referencing vol1_ds ({np.shape(vol1_ds)}) and vol2_ds ({np.shape(vol2_ds)})')
    else:
        assert n_1[0] == n_2[0], "ERROR"
        vol1_ds = vol1
        vol2_ds = vol2
        # larger_vol = max(n_1[0], n_2[0])
        # smaller_vol = min(n_1[0], n_2[0])
        # if downsample is None:
        #     log.info(f'---> Downsampling larger volume from {larger_vol} to {smaller_vol} pixels')
        #     if n_1[0] > n_2[0]:
        #         vol1_ds = cryo_downsample(vol1, (smaller_vol, smaller_vol, smaller_vol))
        #         vol2_ds = vol2
        #     else:
        #         vol1_ds = vol1
        #         vol2_ds = cryo_downsample(vol2, (smaller_vol, smaller_vol, smaller_vol))
        # else:
        #     assert downsample <= smaller_vol, "Downsample must be less than the smaller input volume size"
        #     n_ds = downsample
        #     if show_log:
        #         log.info(f'---> Downsampling volumes from {n_1[0]}, {n_2[0]} to {n_ds} pixels')
        #     if n_1[0] > n_2[0]:
        #         if downsample == smaller_vol:
        #             vol1_ds = cryo_downsample(vol1, (downsample, downsample, downsample))
        #             vol2_ds = vol2
        #         else:
        #             vol1_ds = cryo_downsample(vol1, (downsample, downsample, downsample))
        #             vol2_ds = cryo_downsample(vol2, (downsample, downsample, downsample))
        #     else:
        #         # that means n_1[0] < n_2[0]
        #         if downsample == smaller_vol:
        #             vol1_ds = vol1
        #             vol2_ds = cryo_downsample(vol2, (downsample, downsample, downsample))
        #         else:
    ...
    # if downsample is None:
    #     # then we need to assure vol1 dimension == vol2 dimension:
    #     if n_1[0] == n_2[0]:
    #         n_ds = n_1[0]
    #     else:
    #         n_ds = 64
    #         assert n_1[0] == n_2[0], "Input volumes have different dimensions, please choose downsample value"
    #     vol1_ds = vol1
    #     vol2_ds = vol2
    # else:
    #     n_ds = min(n_1[0], n_2[0], downsample)
    #     if show_log:
    #         log.info(f'---> Downsampling volumes from {n} to {n_ds} pixels')
    #     vol1_ds = cryo_downsample(vol1, (n_ds, n_ds, n_ds))
    #     vol2_ds = cryo_downsample(vol2, (n_ds, n_ds, n_ds))
    ...
    if align_in_place:
        ...
        vol2_original_ds = cryo_downsample(vol2_original, (downsample, downsample, downsample))
        R_est, R_est_J = fast_alignment_3d(sym, vol1_ds.copy(), vol2_original_ds.copy(), Nprojs, trueR, G_c, refrot, verbose, log, show_log)
    ...
    if show_log:
        ...
        log.info('---> Applying estimated rotation to volumes')
    ...
    if vol2_original is None:
        ...
        # query_vol_ds = cryo_crop(vol2aligned, crop_out_3d)
        # query_vol_cropped = cryo_downsample(query_vol_ds, (N_ref, N_ref, N_ref))
        vol2aligned = query_vol_cropped

    if show_log:
        log.info('---> Estimating shift for volumes')

    # log.info(f'vol11.shape = {vol1.shape}')
    # log.info(f'vol2aligned.shape = {vol2aligned.shape}')

    bestdx = register_translations_3d(vol1, vol2aligned)
    logger.debug("bestdx=%s", str(bestdx))
    log.info(f'---> Translations BEFORE pixel factor: [{bestdx[0]:.3f}, {bestdx[1]:.3f}, {bestdx[2]:.3f}]')

    # if p is not None:
    #     pixel_factor = p
    #     bestdx = bestdx * pixel_factor
    #     log.info(f'---> Translations AFTER pixel factor: [{bestdx[0]:.3f}, {bestdx[1]:.3f}, {bestdx[2]:.3f}]')
    ...
    if align_in_place:
        ...
        if show_log:
            log.info('---> Computing correlations of volumes')
    ...
    #         log.info(f'{aligned_bestR[2, 0]:.4f} {aligned_bestR[2, 1]:.4f} {aligned_bestR[2, 2]:.4f}')
    #         log.info('Estimated symmetry element:')
    #         log.info(f'{g_est[0, 0]:.4f} {g_est[0, 1]:.4f} {g_est[0, 2]:.4f}')
    #         log.info(f'{g_est[1, 0]:.4f} {g_est[1, 1]:.4f} {g_est[1, 2]:.4f}')
    ...
    #         log.info(f'Angle between axes is {math.degrees(np.arccos(np.dot(axis_est, axis_ref))):.4f}  degrees')
    #         log.info('In-plane rotation:')
    #         log.info(f'Reference {math.degrees(angle_ref):.4f} degrees')
    #         log.info(f'Estimated {math.degrees(angle_est):.4f} degrees')
...
def align_volumes(vol1, vol2, verbose=0, opt=None, show_log=True, session=None):
    ...
    if threshold:
        vol1_inds_above_threshold = np.where(vol1_ds.ravel() >= threshold)[0]
        vol2_aligned_inds_above_threshold = np.where(vol2_aligned_ds.ravel() >= threshold)[0]
        vol2_aligned_J_inds_above_threshold = np.where(vol2_aligned_J_ds.ravel() >= threshold)[0]
        inds_to_keep = []
        inds_to_keep_J = []
        # log.info(f"vol1_inds_above_threshold={vol1_inds_above_threshold}")
        # log.info(f"vol1_inds_above_threshold.shape={vol1_inds_above_threshold.shape}")
        for i in vol1_inds_above_threshold:
            if i in vol2_aligned_inds_above_threshold:
                inds_to_keep.append(i)
            if i in vol2_aligned_J_inds_above_threshold:
                inds_to_keep_J.append(i)

        inds_to_keep = np.array(inds_to_keep)
        inds_to_keep_J = np.array(inds_to_keep_J)

        vol1_flat = vol1_ds.ravel()[inds_to_keep]
        vol2_aligned_flat = vol2_aligned_ds.ravel()[inds_to_keep]

        vol1_J_flat = vol1_ds.ravel()[inds_to_keep_J]
        vol2_aligned_J_flat = vol2_aligned_J_ds.ravel()[inds_to_keep_J]
    else:
        vol1_flat = vol1_ds.ravel()
        vol2_aligned_flat = vol2_aligned_ds.ravel()

        vol1_J_flat = vol1_ds.ravel()
        vol2_aligned_J_flat = vol2_aligned_J_ds.ravel()

    no1 = np.mean(np.corrcoef(vol1_flat, vol2_aligned_flat, rowvar=False)[0, 1:])
    no2 = np.mean(np.corrcoef(vol1_J_flat, vol2_aligned_J_flat, rowvar=False)[0, 1:])
    logger.debug("no1=%f", no1)
    logger.debug("no2=%f", no2)

    print_to_log(log, f"no1 = {no1}")
    print_to_log(log, f"no2 = {no2}")
