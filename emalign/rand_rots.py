...
def qrand(k):
    ...
    # Each quaternion is a four-elements column vector. Returns a matrix of size 4xK.
    ...
    # The antipodal points (-a,-b,-c,-d) and (a,b,c,d) are identified as the same group elements, so we take a>=0.
...
def q_to_rot(q):
    ...
    #   Input:
    #       q - quaternion. Can be a vector of dimensions 4 x n.
    #   Output:
...
def rand_rots(n):
    ...
    # Output:
    #    rot_matrices - an array of 3x3xn containing n rotation matrices sampled from the uniform distribution on SO(3).
