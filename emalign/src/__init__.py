...
# from .emalign_cmd import register_emalign_command
...
class _EMalignBundle(BundleAPI):
    ...
    def register_command(command_name, logger):
        ...
        # This method is called once for each command listed in bundle_info.xml.
        # Since we only listed one command, we expect only a single call to this method.

        # We import the function to call and its argument description from the ``cmd`` module, adding a synopsis from
        # bundle_info.xml if none is supplied by the code.

        # We then register the function as the command callback with the chimerax.core.commands module.
        # Note that the command name registered is not hardwired, but actually comes from bundle_info.xml.
