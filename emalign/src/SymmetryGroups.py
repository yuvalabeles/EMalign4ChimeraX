...
def genSymGroup(sym):
    ...
    if s == 'C':
        ...
        G[8, :, :] = np.array(
            [[0, -1, 0], [0, 0, -1], [1, 0, 0]])  # axis: [-1, 1,-1] angle: 240    G(8) = G(7)^T
    ...
    # %%
        ...
        # 2pi/3 and 4pi/3:  
        G[25, :, :] = R.from_rotvec(
            (2 * np.pi) / 3 * np.array([0, -1, phi ** 2] / LA.norm([0, -1, phi ** 2]))).as_matrix()
        G[26, :, :] = R.from_rotvec(
            (4 * np.pi) / 3 * np.array([0, -1, phi ** 2] / LA.norm([0, -1, phi ** 2]))).as_matrix()
        ...
        G[29, :, :] = R.from_rotvec(
            (2 * np.pi) / 3 * np.array([-1, phi ** 2, 0] / LA.norm([-1, phi ** 2, 0]))).as_matrix()
        G[30, :, :] = R.from_rotvec(
            (4 * np.pi) / 3 * np.array([-1, phi ** 2, 0] / LA.norm([-1, phi ** 2, 0]))).as_matrix()
        ...
        G[33, :, :] = R.from_rotvec(
            (2 * np.pi) / 3 * np.array([phi ** 2, 0, -1] / LA.norm([phi ** 2, 0, -1]))).as_matrix()
        G[34, :, :] = R.from_rotvec(
            (4 * np.pi) / 3 * np.array([phi ** 2, 0, -1] / LA.norm([phi ** 2, 0, -1]))).as_matrix()
